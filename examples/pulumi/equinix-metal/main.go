...
func main() {
	pulumi.Run(func(ctx *pulumi.Context) error {
		...
		_, err = talos.NewNodeBootstrap(ctx, "bootstrap-cp-0", &talos.NodeBootstrapArgs{
			...
			TalosConfig: clusterSecrets.TalosConfig,
		})
		if err != nil {
			return err
		}
		...
		kubeconfig := pulumi.All(controlPlaneNodeIPs[0].ToStringOutput(), clusterSecrets.TalosConfig).ApplyT(func(args []interface{}) (string, error) {
			ip := args[0].(string)
		...
		ctx.Export("talosConfig", clusterSecrets.TalosConfig)
